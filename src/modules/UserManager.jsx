...
export const register = (user) => {
  return registerWithFirebase(user.email, user.password)
    ...
    .then(newUserFromJason => {
      setUserInLocalStorage(newUserFromJason)
      return newUserFromJason
    })
    .catch(error => {
      alert("ERROR guys error", error)
    })
}
...
export const login = (email, password) => {
  return loginWithFirebase(email, password)
    .then(firebaseId => (getUser(firebaseId))
      .then(userToJsonServer => {
        setUserInLocalStorage(userToJsonServer)
        return userToJsonServer
      }))
}
